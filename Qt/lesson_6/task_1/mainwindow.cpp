...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        auto calculateRace1 = [this]{concurRace1->DoWork(&number, false, ui->sb_initNum->value());};
        auto calculateRace2 = [this]{concurRace2->DoWork(&number, false, ui->sb_initNum->value());};

        QtConcurrent::run(calculateRace1).then(calculateRace2);
    }
    else{
